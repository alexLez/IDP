...
void find_current_colour_and_show(int something1,int red1, int yellow1,int something2,int red2, int yellow2,int something3,int red3, int yellow3){
  //find the current colour
        ...
        rolling_colour2[9] = light_levels(Pin2);
        rolling_colour3[9] = light_levels(Pin3);
        ...
        if ((instantaneous_reading_average1>rolling_average1+something1)){//if there is anything detected, slow down and check it  ){  //if the sensor readings for any of the LDRs jumps up, slow down and approach
             ...
             float light_levels_used2=light_levels(Pin2);  //measure the light levels here for sensor 2
             float light_levels_used3=light_levels(Pin3);  //measure the light levels here for sensor 3
             if ((light_levels_used1>BlackRef1+yellow1)){    //if any of them are yellow we say it is yellow
              colour = 2 ;
              Serial.println("Yellow");//say it is yellow
              Serial.println(analogRead(Pin1));//print colour value for testing
              digitalWrite(44,HIGH);//flash LED
              delay(3000);
              digitalWrite(44,LOW);
             }
             else if (light_levels_used1>BlackRef1+red1) {   //red if any of them are red
